...
test(
  'creating a task sends a request matching CreateTaskRequest',
  { tag: '@mobile' },
  async ({ page }) => {
...
test(
  'editing a task sends a request matching UpdateTaskRequest',
  { tag: '@mobile' },
  async ({ page }) => {
...
test(
  'posting a comment sends a request matching CreateCommentRequest',
  { tag: '@mobile' },
  async ({ page }) => {
    await mockJson(page, 'GET', '/api/tasks/task-1', fakeTask)
    await mockJson(page, 'GET', '/api/tasks/task-1/comments', [])

...
**Running the e2e suite**: the backend must already be up (`docker compose up -d` + `dotnet run --project src/TaskFlow.Api` from `server/`, see `server/CLAUDE.md`) — Playwright does not start it. For the frontend, `playwright.config.ts` auto-starts `npm run preview` on port 4173 against whatever's in `dist/` (run `npm run build` first) unless `PLAYWRIGHT_BASE_URL` is set, in which case that URL (e.g. a Vercel preview) is used as-is and nothing is started locally. Fixtures talk to the backend directly via `E2E_API_URL` (defaults to `http://localhost:5151`, i.e. the same backend `VITE_API_URL` points the built app at).
